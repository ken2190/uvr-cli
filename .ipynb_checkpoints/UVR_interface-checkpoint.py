...
class UVRInterface:
    ...
    def assemble_model_data(self, ensemble_model_manual_list=None, model=None, arch_type=ENSEMBLE_MODE, is_dry_check=False) -> List[ModelData]:
        ...
        if arch_type == ENSEMBLE_MODE:
            print(ensemble_model_manual_list)
            model_data: List[ModelData] = [ModelData(model_name) for model_name in ensemble_model_manual_list]
    ...
    def process(self, model_name, ensemble_model_manual_list, arch_type, audio_file, export_path, is_model_sample_mode=False, is_4_stem_ensemble=False, set_progress_func=None, console_write=print) -> SeperateAttributes:
        stime = time.perf_counter()
        time_elapsed = lambda:f'Time Elapsed: {time.strftime("%H:%M:%S", time.gmtime(int(time.perf_counter() - stime)))}'

        if arch_type==ENSEMBLE_MODE:
            model_list, ensemble = self.assemble_model_data(ensemble_model_manual_list), Ensembler()
        ...
        if not is_ensemble:
            ...
            print(model.model_basename)
            print(os.path.splitext(os.path.basename(audio_file)))
